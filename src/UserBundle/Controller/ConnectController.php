<?php
	...
	class ConnectController extends LayoutFormInterfaceController {
		/**
		...
		public function connectAction(Request $request) {
			...
			if ($connect
				...
				return new RedirectResponse($this->generate('user_oauth_connect_registration', array('key' => $key)));
			}
		...
		public function registrationAction(Request $request, $key) {
			...
			$form = $this->container->get('form.factory')->create($this->container->get('user_oauth.registration.form'), null, array());

			$formHandler = $this->container->get('user_oauth.registration.form.handler');



			if ($formHandler->process($request, $form, $userInformation)) {
				$this->container->get('user_oauth.account.connector')->connect($form->getData(), $userInformation);
				...
				$content = new Twig('user_connect_registration_success', array(
			...
			$content = new Twig('user_connect_registration', array(
				'key'             => $key,
		...
		public function connectServiceAction(Request $request, $service) {
			...
			if ($request->isMethod('POST')) {
				...
				if ($form->isValid()) {
					...
					$content = new Twig('user_connect_connect_success', array(
						'userInformation' => $userInformation,
						'service'         => $service,
					));
